...
CreateIOQContigPoll_r10b::CreateIOQContigPoll_r10b(int fd, string grpName, string testName) :
    Test(fd, grpName, testName, SPECREV_10b)
{
    // 66 chars allowed:     xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    mTestDesc.SetCompliance("revision 1.0b, section 7");
    mTestDesc.SetShort(     "Create IOCQ and IOSQ's");
    // No string size limit for the long description
    mTestDesc.SetLong(
        "Issue the admin commands Create I/O SQ and Create I/Q CQ to the "
        "ASQ and reap the resulting CE's from the ACQ to certify those Q's "
        "have been created.");
}
    ...
    SharedACQPtr acq)
{
    ...
    SharedIOCQPtr iocq = CAST_TO_IOCQ(
        gRsrcMngr->AllocObj(Trackable::OBJ_IOCQ, IOCQ_GROUP_ID));
    ...
    SharedACQPtr acq)
{
    ...
    SharedIOSQPtr iosq = CAST_TO_IOSQ(
        gRsrcMngr->AllocObj(Trackable::OBJ_IOSQ, IOSQ_GROUP_ID));
