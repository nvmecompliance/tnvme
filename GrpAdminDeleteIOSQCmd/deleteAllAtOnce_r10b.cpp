...
DeleteAllAtOnce_r10b::DeleteAllAtOnce_r10b(int fd, string mGrpName,
    string mTestName, ErrorRegs errRegs) :
    Test(fd, mGrpName, mTestName, SPECREV_10b, errRegs)
{
    // 63 chars allowed:     xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    mTestDesc.SetCompliance("revision 1.0b, section 5");
    mTestDesc.SetShort(     "Delete all IOSQ's at the same time");
    ...
    mTestDesc.SetLong(
        ...
        "Issue X DeleteIOSQ cmds and make sure the doorbell is rung for "
        "all cmds at once. Expect to reap all X cmds successfully. Verify "
        "CE.CID with expected value.");
}
