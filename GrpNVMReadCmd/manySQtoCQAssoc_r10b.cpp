...
void
ManySQtoCQAssoc_r10b::SetWriteCmd(SharedWritePtr *writeCmd)
{
    ...
    vector<uint16_t> mSQIDToSQHDVector)
{
    ...
    for (uint16_t i = 0; i < numTil; i++) {
        ...
        // Verify each SDID has arrived only once.
        for (uint16_t it = 0; it < mSQIDVector.size(); it++) {
            if (mSQIDToSQHDVector[it] == ce.n.SQID) {
                LOG_ERR("The SQID %d in CEs shouldn't be more than once",
                    ce.n.SQID);
                throw exception();
            } else {
                mSQIDVector.push_back(ce.n.SQID);
            }
        }
    ...
    for (uint16_t it = 0; it < mSQIDToSQHDVector.size(); it++) {
        if (mSQIDToSQHDVector[it] != USHRT_MAX) {
            LOG_ERR("The SQID %d in CE is invalid", it);
            throw exception();
        }
    }
}
